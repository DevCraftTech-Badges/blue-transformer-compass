...
export interface IRRow { // Insulation Resistance Measurement row (spec: only 3 inputs per row)
  vdc?: string; m1?: string; m10?: string; // Vdc, MΩ at 1st, MΩ at 10th
}

export interface PFRow { // Power Factor & Capacitance row (spec: 5 inputs per row + read-only remark)
  current?: string; watt?: string; pf?: string; cor20?: string; cap?: string; remark?: string; // remark read-only display
}

export interface AutoTransformerInsulationData {
  general: GeneralInfo;
  irRows: IRRow[]; // 2 rows
  pfHeader: { cor20?: string; testKV?: string; };
  pfRows: PFRow[]; // 4 input rows
}

export interface AutoTransformerInsulationMeasurementFormProps {
  initialData?: Partial<AutoTransformerInsulationData> & { viewOnly?: boolean };
  onSubmit: (data: AutoTransformerInsulationData) => void;
  onCancel: () => void;
  viewOnly?: boolean;
}

const buildInitial = (initial?: AutoTransformerInsulationMeasurementFormProps['initialData']): AutoTransformerInsulationData => ({
...
// -------- Component --------
const AutoTransformerInsulationMeasurementForm: React.FC<AutoTransformerInsulationMeasurementFormProps> = ({ initialData, onSubmit, onCancel, viewOnly }) => {
  const [data, setData] = useState<AutoTransformerInsulationData>(() => buildInitial(initialData));
  ...
  const updateGeneral = (f: keyof GeneralInfo, v: string) => setData(p => ({ ...p, general: { ...p.general, [f]: v } }));
  const updateIr = (idx: number, f: keyof IRRow, v: string) => setData(p => ({ ...p, irRows: p.irRows.map((r,i)=> i===idx? { ...r, [f]: v }: r ) }));
  const updatePfHeader = (f: 'cor20' | 'testKV', v: string) => setData(p => ({ ...p, pfHeader: { ...p.pfHeader, [f]: v } }));
  const updatePfRow = (idx: number, f: keyof PFRow, v: string) => setData(p => ({ ...p, pfRows: p.pfRows.map((r,i)=> i===idx? { ...r, [f]: v }: r ) }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      {/* Top Table: Insulation Resistance Measurement (spec: 2 rows * 3 numeric inputs + computed PI) */}
  ...
  {/* Bottom Table: Power Factor & Capacitance Measurement (spec: 4 rows * 5 numeric inputs + remark) */}
      <div className="rounded-xl border bg-background/70 backdrop-blur p-5 shadow-sm ring-1 ring-border/50">
...
export default AutoTransformerInsulationMeasurementForm;
